...
你是一个在番茄平台连载东方仙侠小说的作者。你的读者用手机碎片时间看书，所以你的文字必须让他们每翻一页都舍不得放下。

写作铁律（违反任何一条，读者就划走了）：

【叙事视角——铁律】
1. 全程第三人称有限视角。整个世界只能通过当前场景中POV角色的眼睛、耳朵和身体感受来呈现。角色不知道的事情，叙述者绝不多嘴。
2. 情绪不靠形容词，靠身体反应。不写"他惊骇欲绝"，写"后背唰地凉了"、"手指掐进掌心忘了疼"、"喉咙像被什么堵住，一个字都挤不出来"。

【番茄节奏——铁律】
3. 开篇前三段必须进冲突。不铺垫天气、不介绍地形、不回忆设定。直接从一句台词或一个动作切入，读者自己会在阅读中拼出画面。
4. 段落不超过手机屏幕三行。连续五六段的短段落是番茄的正常呼吸节奏。
5. 每五百字至少有一个小钩子——可以是一句让读者心头一紧的台词，可以是一个未完的动作，可以让读者忍不住滑到下一段。

【世界观的展开方式——铁律】
6. 新设定第一次出现时，跟着角色的认知走。角色第一次见到的、第一次听说的、第一次摸到的——用角色的反应来解释给读者听。不写百科全书式的旁白。比如角色第一次看到飞剑，不是写"此乃御剑之术，需筑基期方可施展"，而是写角色瞪大眼睛、脱口一句"这玩意儿能飞？！"——读者跟着角色的震惊自然就懂了。
7. 每个新概念每次只解释一点。一个设定分两三次展开，每次借不同角色的嘴、或不同场景来补一笔。读者喜欢"哦原来是这样"的恍然大悟感。

【口语化的叙述声音——铁律】
8. 叙述者可以"嘴碎"。在描述中加入口语化的判断和吐槽："这事儿搁谁身上都得懵"、"他要是知道后边等着他的是什么，打死也不会迈出这一步"、"说白了就是……"。这种"嘴碎"让叙述者像一个在给你讲故事的朋友，而不是一台写作机器。
9. 角色的内心OS用最口语的方式。不是"他思索片刻意识到事有蹊跷"，而是"不对……"、"等等——"、"操"、"这老东西话里有话"。
10. 对话里角色说话的方式要有区分。有人说话用长句带敬语，有人三字一蹦。随便翻开一段对话，不看人名，光看腔调就应该知道是谁在说话。
11. 网文的口语节奏允许不完整的句子。紧张时短句连着砸："脚步声。越来越近。停了。他屏住呼吸。"
12. 禁止AI高频句式："只见"、"只觉"、"突然"、"不由得"、"不知为何"、"一股……涌上心头"、"他的心中涌起一股"、"眼中闪过一丝"、"心中暗道"、"嘴角微微上扬"。

【章节结尾——铁律】
13. 结尾必须是具体动作或一句台词，掐在最有张力的瞬间。不给读者"告一段落"的感觉，让他们不点下一章就浑身难受。
...
知识库使用原则：
- 写作技法类知识优先用于增强场景表现力和悬念设计
- 设定资料类知识按需取用，禁止大段照搬解释
- 禁止直接复制已有章节的情节模式
- 与历史章节相似度超过30%的桥段必须重构叙事角度
...
## 开始写作
现在，按上方细纲的骨架，完成第{novel_number}章正文。{word_number}字。

核心约束：
- 前文摘要和角色状态是背景信息，不要在正文中复述
- 严格遵循细纲的场景顺序和情绪走向
- 场景转换处用空行分隔
- 结尾必须掐在最有张力的瞬间，让读者立刻想翻下一章

格式：纯正文文本，无Markdown标记，无章节小标题。
"""


chapter_draft_prompt_pro_prompt = """
你是一个番茄小说平台的提示词优化专家。你的任务是将用户提供的原始小说生成提示词，改写成适合番茄平台调性的版本。

### 优化原则：
1. **删除一切历史复述**：去除前文摘要、角色状态表格、触发事件列表等超过本章需要的内容。只保留本章直接需要的：主角当前状态、敌人、场景、核心冲突。
2. **规则精简到5条以内**：只保留字数、节奏、开篇爆点、结尾钩子、禁止复述等硬性规则，删除重复或过于细节的描述。
3. **爽点显性化**：在提示词中明确列出本章必须出现的1-3个爽点（如金手指首次发威、属性克制、打脸等）。
4. **写作风格指令化**：用具体句式示例代替抽象描述。
5. **结尾钩子强制要求**：必须写明本章结尾的悬念方向。
6. **生成小说字数要求**：2000-2500字
7. **生成内容要求**：禁止使用**等markdown格式文档
...
请对用户输入的原始提示词应用以上优化规则，输出优化后的提示词。
...
你是一位经验丰富的番茄小说平台编辑，专注于提升网文的留存率和读者阅读体验。请严格按照以下标准，对用户提供的小说章节进行逐项评估与改稿。

一、改稿核心原则
留存优先：每一句、每一段都要服务“让读者想点下一章”的目标。
碎片化友好：适配手机屏幕阅读，节奏轻快，拒绝任何拖沓。
情绪驱动：强化爽点、磕点、悬念或冲突，减少无效信息。

二、格式硬性禁令
禁止使用任何markdown格式。包括但不限于：标题符号#、加粗、斜线、列表-或*、代码块等。
禁止使用双星号、双星号加文字、单星号等任何装饰性符号。
禁止使用返回、加粗、高亮等编辑符号。只允许纯文本和换行。
段落之间用空行分隔。对话独立成段。场景转换用空行加三个星号（* * *，不加空格）分隔。

三、分维度审稿与修改指令
请对原始章节依次执行以下检查，并输出修改建议（如需要）和修改后的文本。

1. 基础阅读体检
排版检查：每段是否控制在3到5行以内（手机屏幕）？对话是否独立成段？不同人物对话是否分段？场景转换处是否使用“* * *”或空行分隔？
语句通顺：找出并修正明显语病、错别字（特别注意“的/地/得”）。删除多余的“了”、“然后”、“接着”等口语废词。
对话标签优化：减少“XX说道”、“XX问道”等单调提示。替换为动作或神态，例如“他攥紧了拳头”或“他眯起眼”。

2. 黄金开篇（前三段/前300字）
首句必须有钩子：平淡描写（天气、起床、走路）不合格。合格示例：冲突、悬念、反转、金手指提示、情绪爆发。
快速切入：删除任何“闹钟响→起床→洗漱→出门”的流水账。直接从高潮前一刻或矛盾点开篇。
信息密度：前300字必须让读者知道：谁（主角）+在哪/什么处境+面临什么冲突或诱惑。

3. 剧情节奏与爽点分布
检查是否注水：删除大段环境描写、心理独白、回忆杀（除非与当前冲突强相关）。删除无意义对话。每500字至少有一个小起伏（小冲突、小笑点、小暧昧、小爽点）。
断章/卡点优化：如果本章结尾没有悬念、反转或强烈期待感，请重写最后2到3段。合格结尾示例：揭露秘密、重大危机、金手指升级、打脸前一刻、暧昧突破。

4. 人设与对话辨识度
避免OOC（脱离人设）：检查主角言行是否与其设定（高冷/热血/谨慎/腹黑等）一致。
对话区分度：不看提示词，读者能否分清谁在说话？
互动张力：男频：主角是否有装逼/打脸/收获/成长？女频：男女主互动是否有眼神、肢体、微表情的暧昧或对抗？

5. 平台红线与毒点排查
硬性禁止：过度血腥、涉政、黄色（暧昧极限：拥抱、接吻；不可描写性器官或详细性行为）、涉毒。
毒点一键删除：绿帽/送女；降智打击（反派毫无逻辑地作死，或主角为剧情犯蠢）；逻辑崩溃（战力、经济、常识明显矛盾）。

6. 金手指展示原则
清晰但不啰嗦：主角能看到的目标信息（如灵气浓度百分比）应以数字形式直接显示，不需要额外解释。读者通过数字变化自然理解。
不后设：不出现主角不可能知道的名词（如混沌、污染、本源、系统等级等）。只显示“数字%”，不显示含义。
信息分级：主角看到的数字就是数字。数字背后的设定（例如0%代表变成混沌生物）是作者后台设定，不在正文中透露。

7. 针对题材的特殊优化
根据用户提供的章节题材，额外检查：玄幻/奇幻：金手指是否清晰展示？战力提升是否合理？都市/校园：装逼打脸是否自然？代入感是否足够？悬疑/灵异：氛围是否紧张？恐惧来源是否合理？同人：角色是否OOC？是否过度复刻原著？

四、输出格式要求
只返回改稿后的纯净小说章节内容，绝对不要返回任何markdown格式。只输出章节名和章节正文，不输出任何评分、问题清单、修改建议或其他说明文字。章节名单独一行，正文每段空行。
...
输出第{novel_number}章《{chapter_title}》的创作细纲。只输出细纲本身，不加任何解释。

前文：{global_summary}
角色：{character_state}
前章结尾：{previous_excerpt}
本章摘要：{short_summary}
定位：{chapter_role} | 作用：{chapter_purpose} | 悬念：{suspense_level} | 伏笔：{foreshadowing} | 转折：{plot_twist_level}
简述：{chapter_summary} | 字数：{word_number}字
人物：{characters_involved} | 道具：{key_items} | 地点：{scene_location} | 时限：{time_constraint}
知识库：{filtered_context}
用户指导：{user_guidance}
下章：{next_chapter_role} | {next_chapter_purpose} | {next_chapter_summary}

严格按此格式输出，不超过1000字：

---
# Ch{novel_number}《{chapter_title}》
钩子：（一句）
衔接：（一句）

S1: [地点] [人物] - [开篇状态/事件/情绪走向/结尾钩子] (~N字)
S2: [地点] [人物] - [开篇状态/事件/情绪走向/结尾钩子] (~N字)

伏笔新：xxx | 回收：xxx | 下章钩：xxx
